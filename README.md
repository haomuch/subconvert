...
- ⚡ **边缘缓存加速** — 转换结果在 Cloudflare 边缘缓存 5 分钟，源订阅变更后最长 5 分钟生效
...
转换结果会在 Cloudflare 边缘缓存 **5 分钟**。因此：

- 源订阅变更后，最长 5 分钟才会反映到你的订阅链接上；
- 删除链接会立即清掉当前边缘节点的缓存，但其他边缘节点上的副本最多还会存活 5 分钟。

如果想让变更立刻生效，可以在创建时换一个新的自定义路径。
